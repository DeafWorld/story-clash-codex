...
export async function POST(
  ...
  if (!limit.allowed) {
    logWarn("ask.flag_rate_limited", { userId });
  ...
  if (flagError) {
    ...
    logError("ask.flag_failed", { error: flagError.message });
  ...
  if (error) {
    logError("ask.flag_missing", { questionId: params.id });
  ...
  logEvent("ask_question_flagged", { questionId: params.id, hidden });
