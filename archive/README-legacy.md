...
## $0 Cloudflare Backend (Always-On Beta)

The repository now includes a Cloudflare Worker + Durable Object backend in `/cloudflare`:
...
Set frontend env vars for Worker mode:

- `NEXT_PUBLIC_REALTIME_TRANSPORT=ws`
- `NEXT_PUBLIC_API_BASE_URL=https://<worker>.workers.dev`
- `NEXT_PUBLIC_WS_BASE_URL=wss://<worker>.workers.dev`

For local fallback with the Node Socket.IO server, keep:
