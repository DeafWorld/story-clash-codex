...
export async function GET(request: Request, context: { params: Promise<{ code: string }> }) {
  ...
  try {
    ...
    const narratorLine = (recap.latestNarration?.text ?? recap.narrationLog.at(-1)?.text ?? "The story got intense.").slice(
      0,
      120
    );
    const shareText = `We just played Story Clash. Ending: ${recap.endingType.toUpperCase()}. ${narratorLine} Join the next run.`;
