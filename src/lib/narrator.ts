...
import type { EndingType, GenreId, NarrationLine, NarrationTone, NarrationTrigger } from "../types/game";
...
const NARRATOR_MODE = (process.env.NARRATOR_MODE ?? "local").trim().toLowerCase();
...
const CHOICE_BANK: Record<NarrationTone, string[]> = {
  calm: [
    "{player} commits to {choice}.",
    "{player} keeps it steady with {choice}.",
  ],
  uneasy: [
    "{player} chooses {choice}, and the mood tilts.",
    "A careful gamble: {player} goes with {choice}.",
  ],
  urgent: [
    "No hesitation. {player} locks in {choice}.",
    "Clock ticking, {player} fires off {choice}.",
  ],
  desperate: [
    "At the brink, {player} throws everything at {choice}.",
    "Under full pressure, {player} calls {choice}.",
  ],
  hopeful: [
    "The team leans in as {player} picks {choice}.",
    "Momentum swings when {player} selects {choice}.",
  ],
  grim: [
    "{player} takes the hard road: {choice}.",
    "The cost is visible as {player} chooses {choice}.",
  ],
};

const TIMEOUT_BANK: Record<NarrationTone, string[]> = {
  calm: [
    "Silence wins the turn. Fate makes the move.",
    "No call in time. The story chooses for the crew.",
  ],
  uneasy: [
    "The clock steals the turn. Random fate steps in.",
    "Hesitation costs dearly. Chance decides this beat.",
  ],
  urgent: [
    "Timer hit zero. Control slips to chaos.",
    "Too late. The room answers with a random move.",
  ],
  desperate: [
    "Time is up. Panic takes the wheel.",
    "The deadline snaps shut. Survival mode picks for you.",
  ],
  hopeful: [
    "A missed moment, but the story still moves.",
    "The turn times out. There is still a path ahead.",
  ],
  grim: [
    "No decision landed. The dark made one anyway.",
    "The timer dies, and the story turns colder.",
  ],
};

const ENDING_BANK: Record<EndingType, string[]> = {
  triumph: [
    "Against the odds, the crew carves out a win.",
    "The plan holds. Hope survives this chapter.",
  ],
  survival: [
    "Barely breathing, but still standing. The crew lives on.",
    "It is not pretty, but the crew escapes the worst.",
  ],
  doom: [
    "The night takes its payment. This run ends in ruin.",
    "The story closes hard. Nobody forgets this ending.",
  ],
};
...
export function generateNarrationLine(context: NarrationContext): NarrationLine {
  ...
  const rendered = trimToLength(
    normalizeSpacing(applyTemplate(template, { player, choice })),
